...
export class ReservationsComponent {
  ...
  searchSynonyms() {
    this.reservationService.getAllReservation().subscribe((res) => {
      this.reservations = res.filter((reservation: any) =>
      reservation.nom_reserv.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
...
fetchStatistics(): void {
  this.reservationService.getMonthlyReservationCounts(this.year)
    .subscribe({
      next: (data: any) => { // Assurez-vous que le type ici correspond à celui retourné par le service
        if (Array.isArray(data)) { // Vérifiez si les données sont bien un tableau
